...
use crate::{check_permissions, methods::Action};
...
pub async fn initialize(
    ...
    Kiosk::fetch_by_id(&kiosk.last_insert_id, session, &db.0)
        .await
        .map(|v| Json(v))
}
...
pub async fn update(
    ...
    Kiosk::update(input_data.data(), session, id, &conn.0)
        .await
        .into()
}
...
pub async fn update_preferences(
    ...
    Kiosk::update_preferences(id, session, input_data.data(), &db.0)
        .await
        .into()
}
...
#[openapi(tag = "Kiosk")]
#[post("/online/<id>")]
pub async fn update_online_status(db: InternalDb, id: &str, session: Session) -> Convert<Kiosk> {
    check_permissions!(session.clone(), Action::FetchKiosk);
...
pub async fn auth_log(
    ...
    check_permissions!(session.clone(), Action::AccessAdminPanel);
    Kiosk::auth_log(id, session, input_data.data(), &db.0)
        .await
        .map(|_| ())
}
