...
#[openapi(tag = "Product")]
#[get("/<id>")]
pub async fn get(session: Session, db: InternalDb, id: i32) -> Convert<Product> {
    check_permissions!(session.clone(), Action::FetchProduct);
    Product::fetch_by_id(&id.to_string(), session, &db.0)
        .await
        .into()
}
...
pub async fn get_with_associated_promotions(
    ...
    check_permissions!(session.clone(), Action::FetchProduct);
    Product::fetch_by_id_with_promotion(&id.to_string(), session, &db.0)
        .await
        .into()
}
...
#[openapi(tag = "Product")]
#[get("/name/<name>")]
pub async fn get_by_name(db: InternalDb, session: Session, name: &str) -> Convert<Vec<Product>> {
    check_permissions!(session.clone(), Action::FetchProduct);
...
pub async fn get_by_name_exact(
    ...
    check_permissions!(session.clone(), Action::FetchProduct);
    Product::fetch_by_name_exact(name, session, &db.0)
        .await
        .into()
}
...
#[openapi(tag = "Product")]
#[get("/search/<query>")]
pub async fn search_query(db: InternalDb, session: Session, query: &str) -> Convert<Vec<Product>> {
    check_permissions!(session.clone(), Action::FetchProduct);
...
pub async fn search_with_associated_promotions(
    ...
    check_permissions!(session.clone(), Action::FetchProduct);
    Product::search_with_promotion(query, session, &db.0)
        .await
        .into()
}
...
async fn update(
    ...
    Product::update(input_data.data(), session, id, &db.0)
        .await
        .into()
}
...
pub async fn create(
    ...
    let converted: Convert<Product> = Product::fetch_by_id(&data.last_insert_id, session, &db.0)
        .await
        .into();
    converted.0
}
...
#[openapi(tag = "Product")]
#[post("/generate")]
async fn generate(db: InternalDb, session: Session) -> Convert<Vec<Product>> {
...
#[openapi(tag = "Product")]
#[get("/promotion/<id>")]
pub async fn get_promotion(db: InternalDb, session: Session, id: i32) -> Convert<Promotion> {
    check_permissions!(session.clone(), Action::FetchProduct);
    Promotion::fetch_by_id(&id.to_string(), session, &db.0)
        .await
        .into()
}
...
pub async fn get_promotion_by_query(
    ...
    check_permissions!(session.clone(), Action::FetchProduct);
    Promotion::fetch_by_query(query, session, &db.0)
        .await
        .into()
}
...
async fn update_promotion(
    ...
    check_permissions!(session.clone(), Action::ModifyProduct);
    Promotion::update(input_data.data(), session, id, &db.0)
        .await
        .into()
}
...
pub async fn create_promotion(
    ...
    let converted: Convert<Promotion> =
        Promotion::fetch_by_id(&data.last_insert_id, session, &db.0)
            .await
            .into();
    converted.0
}
...
async fn generate_promotion(db: InternalDb, session: Session) -> Convert<Vec<Promotion>> {
    check_permissions!(session.clone(), Action::GenerateTemplateContent);
